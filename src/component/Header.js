 ...
 useEffect(() => {
  async function fetchUser() {
    try {
      ...
      const data = await res.json();
      if (data.user) {
        setUser(data.user);  // this will trigger next effect
      }
    } catch (err) {
      console.error("Fetch user error:", err);
    }
      ...
      <div className="relative flex items-center gap-5">
        ...
        <div className="relative">
          ...
          >
            ...
            {isNotificationOpen && (
  <div className="absolute right-0 mt-4 w-72 bg-white rounded-xl shadow-lg p-3 z-50">
    ...
    <ul className="text-sm text-gray-600 space-y-1 max-h-60 overflow-y-auto">
      {notifications.length > 0 ? (
        ...
        <div className="relative">
          ...
          {/* Dropdown */}
          {isProfileOpen && (
            <div className="absolute top-14 right-0 w-60 bg-white shadow-xl rounded-2xl border border-gray-100 overflow-hidden z-50">
              {/* User Info */}
              <div className="flex items-center gap-3 p-4 bg-gradient-to-r from-blue-50 to-indigo-50">
                <img
                  src="/asset/user_1.png"
              ...
              <div className="p-2 relative">
                ...
                {user?.role === "enduser" && (
                    ...
                    >
                      ...
                      {hoverRequest && (
                        <div className="absolute top-0 ml-2 w-full bg-white rounded-lg shadow-md w-40 text-sm">
                          {["Admin123", "Project Admin", "Super Admin"].map((role) => (
                            <p
                              key={role}
